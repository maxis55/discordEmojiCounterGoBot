...
func (mm *MessageModel) getReactionEmojis(discord *discordgo.Session) ([]EmojiModel, error) {
	...
	if len(mm.Message.Reactions) > 0 {
		for _, reaction := range mm.Message.Reactions {
			...
			reactModels = slices.Concat(reactModels, mm.assignUsersToReactionEmoji(users, reaction.Emoji))
		}
...
func (mm *MessageModel) assignUsersToReactionEmoji(users []*discordgo.User, emoji *discordgo.Emoji) []EmojiModel {
	var res []EmojiModel
	...
	for _, user := range users {
		res = append(res, EmojiModel{
			...
			Timestamp:       mm.Message.Timestamp,
			MessageAuthorId: &mm.Message.Author.ID,
			IsReaction:      true,
			AuthorID:        &user.ID,
		})
	}
...
func rememberNewEmojis(ejs []EmojiModel, db *sql.DB) {
